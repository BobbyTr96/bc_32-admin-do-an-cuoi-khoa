...
const ValidateCourse = ({
  ...
  const currentPageItems = courseApprovals.slice(
    indexOfFirstItem,
    indexOfLastItem
  ); // số item hiện tại của trang
  const totalPageNumber = Math.ceil(courseApprovals.length / itemPerPage); // tổng số trang
  ...
  // func xác thực khóa học
  const handleRegister = async (maKhoaHoc) => {
    try {
      ...
      {courseApprovals.length === 0 ? (
        ...
        <>
          ...
          <div className="validate-course__list my-2 table-responsive">
                  ...
                  <tr key={item.maKhoaHoc}>
                    ...
                    <td>
                      <button
                        ...
                        onClick={() => handleRegister(item.maKhoaHoc)}
                      >
                        Xác thực
                      </button>
                      <button
            ...
            {courseApprovals.length > itemPerPage ? (
              <Pagination
