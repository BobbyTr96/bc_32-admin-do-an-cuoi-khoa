...
const UserAttendedCourse = ({
  ...
  const currentPageItems = userAttendeds.slice(
    indexOfFirstItem,
    indexOfLastItem
  ); // số item hiện tại của trang
  const totalPageNumber = Math.ceil(userAttendeds.length / itemPerPage); // tổng số trang
  ...
  // func hủy khóa học
  const handleDeleteUserAttended = (taiKhoan) => {
    swal({
      ...
      if (response) {
        try {
          ...
          // thực hiện thành công thì request api hàm fetchRegistedCourses đề cập nhật lại danh sách khóa học chờ ghi danh
          fetchUserAttended();
          // hiện thành công
          swal({
      ...
      {userAttendeds.length === 0 ? (
        ...
        <>
          ...
          <div className="validate-course__list my-2 table-responsive">
                  ...
                  <tr key={item.taiKhoan}>
                    ...
                    <td>
                      <button
                        ...
                        onClick={() => handleDeleteUserAttended(item.taiKhoan)}
                      >
                        Hủy
                      </button>
                    </td>
            ...
            {userAttendeds.length > itemPerPage ? (
              <Pagination
...
export default UserAttendedCourse;
